...
IPAddress remoteIP3(192, 168, 158, 128);    // Cambia con l'indirizzo IP del secondo Arduino 
...
void loop() {
  ...
  if(abs(x1-x2)<0.01 && abs(x1-x3)<0.01){
    counter++;
      if(counter==10){
      z = livello;
  ...
  counter_stampa++;  
  // x1=x1-epsilon*((x1-x2))+z-z_pre;

  //Aggiorna x1 con l'algoritmo del consenso
  x1=x1-epsilon*((x1-x2)+(x1-x3))+z-z_pre;
  ...
  while (!new_x_ricevutaa_1 || !new_x_ricevutaa_2) {
