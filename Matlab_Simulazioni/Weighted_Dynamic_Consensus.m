...
media_pes = (w1*z1_values + w2*z2_values + w3*z3_values)/(w1+w2+w3);
...
for k = 1:timesteps-1
    ...
    x1_values(k+1) = x1_values(k) - (1/w1)*epsilon * (x1_values(k) - x2_values(k)) + z1_values(k+1) - z1_values(k);
    x2_values(k+1) = x2_values(k) - (1/w2)*epsilon * ((x2_values(k) - x1_values(k)) + (x2_values(k) - x3_values(k))) + z2_values(k+1) - z2_values(k);   
    x3_values(k+1) = x3_values(k) - (1/w3)*epsilon * (x3_values(k) - x2_values(k)) + z3_values(k+1) - z3_values(k);   
end

% Visualizza i livelli d'acqua
figure;
plot(1:timesteps, x1_values, 'r', 1:timesteps, x2_values, 'b', 1:timesteps, x3_values, 'y', 1:timesteps, media, 'g', 1:timesteps, media_pes, 'k');
xlabel('Time step');
ylabel('Water Level');
